...
for(var i = 0; i < playlist.length; i++) {
    ...
    h3.innerText = playlist[i].name; // <h3>...</h3>
    spanAlbum.innerText = playlist[i].album.name; // <span>...</span>
    spanArtistas.innerText = playlist[i].artists;

    spanAlbum.classList.add('album'); // <span class="album">...</span>
...
pesquisaInput.onkeyup = function() {  
    ...
    for (var i = 0; i < lis.length; i++) {
        ...
        // Pesquisa no titulo a valor que eu digitei
        if (titulo.innerText.toLowerCase().search(this.value.toLowerCase()) > -1){
            console.log('Tem uma musica com esse nome');
        } else {
            console.log('NÂO tem uma musica com esse nome');
